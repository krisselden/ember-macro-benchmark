...
require('yargs')
  .usage('$0 <cmd> [args]')
  .command('add-config [name]', 'Add configuration folder to macro-benchmark repo', {
    name: {
      describe: 'Name of configuration folder. e.g. v3.10.1 or pre-hydration-glimmer.'
    }
  }, addConfig)
  .help()
  .argv
...
function addConfig(options) {
  ...
  if (fs.existsSync(path.join(cwd, 'config'))) {
    ...
    let perProjectConfig = stripIndent`
      module.exports = {
        har: false,
        name: ${name},
        fingerprinted: false
